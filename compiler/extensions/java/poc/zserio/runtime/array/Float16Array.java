 ...
 */
public class Float16Array extends NumericArrayBase<Float>
{
    /**
     ...
     */
    public Float16Array(BitStreamReader reader, int length) throws IOException, ZserioError
    {
     ...
     */
    public Float16Array(int length)
    {
     ...
     */
    public Float16Array(float[] data, int offset, int length)
    {
        if (offset + length > data.length)
            throw new ArrayIndexOutOfBoundsException("Float16Array: Requested array sequence ends beyond " +
    ...
    {
        final Float16Array result = new Float16Array(data.length);
    ...
    {
        return new Float16Array(data, offset, length);
    }
   ...
   @Override
    public int hashCode()
    {
        ...
        for (int index = 0; index < data.length; index++)
            result = result * Util.HASH_PRIME_NUMBER + FloatUtil.convertFloatToShort(elementAt(index));
    ...
    public boolean equals(Object obj)
    {
        ...
        final Float16Array that = (Float16Array)obj;
        if (data.length != that.data.length)
            return false;

        for (int index = 0; index < data.length; index++)
        {
            if (FloatUtil.convertFloatToShort(this.elementAt(index)) !=
                    FloatUtil.convertFloatToShort(that.elementAt(index)))
                return false;
        }
