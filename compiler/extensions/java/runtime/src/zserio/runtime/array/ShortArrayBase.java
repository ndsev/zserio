...
/**
 * Implements base class for Zserio zserio int16 array and for int:9...int:16 arrays and for uint8 array
 * and for bit:1..bit:8 arrays.
 */
abstract class ShortArrayBase extends NumericArrayBase<Short>
{
    /**
     * Empty constructor.
     */
    public ShortArrayBase()
    {}
     ...
     */
    public ShortArrayBase(short[] data, int offset, int length)
    {
        if (offset + length > data.length)
            throw new ArrayIndexOutOfBoundsException("ShortArrayBase: Requested array sequence ends beyond " +
     ...
     */
    public ShortArrayBase(int length)
    {
    ...
    {
        if (obj instanceof ShortArrayBase)
        {
            final ShortArrayBase that = (ShortArrayBase)obj;
    ...
    @Override
    protected void setFromList(List<Short> list)
    {
        data = new short[list.size()];
     ...
     */
    protected void mapImpl(Mapping<Short> mapping, ShortArrayBase result)
    {
        for (int i = 0; i < data.length; i++)
        {
