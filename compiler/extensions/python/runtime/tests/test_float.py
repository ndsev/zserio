...
class FloatUtilTest(unittest.TestCase):
    ...
    def test_uint16_to_float(self):
        ...
        float64_value_max_subnormal = self._create_float64_value(
            0, 1008, 0xFF80000000000
        )  # 2^-15 (1 + 2^-1 + ... + 2^-9)
    ...
    def test_float_to_uint16(self):
        ...
        float64_value_min_underflow_rounding = self._create_float64_value(
            0, 1000, 0x4000000000000
        )  # 2^-23 (1 + 2^-2)
        ...
        float64_value_max32_subnormal = self._create_float64_value(
            0, 896, 0xFFFFFC0000000
        )  # 2^-126 (2^-1 + ... + 2^-23)
    ...
    def _create_float16_value(self, sign, exponent, significand):
        return (
            (sign << self.FLOAT16_SIGN_BIT_POSITION)
            | (exponent << self.FLOAT16_EXPONENT_BIT_POSITION)
            | significand
        )

    def _create_float32_value(self, sign, exponent, significand):
        return (
            (sign << self.FLOAT32_SIGN_BIT_POSITION)
            | (exponent << self.FLOAT32_EXPONENT_BIT_POSITION)
            | significand
        )

    def _create_float64_value(self, sign, exponent, significand):
        return (
            (sign << self.FLOAT64_SIGN_BIT_POSITION)
            | (exponent << self.FLOAT64_EXPONENT_BIT_POSITION)
            | significand
        )
