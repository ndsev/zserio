...
protected:
    void fillSimpleParamTableRow(SimpleParamTable::Row& row, uint32_t id, const std::string& name)
    {
        row.setId(id);
        row.setName(name);
    ...
    void fillSimpleParamTableRows(std::vector<SimpleParamTable::Row>& rows)
    {
        rows.clear();
    ...
    static void checkSimpleParamTableRows(const std::vector<SimpleParamTable::Row>& rows1,
            const std::vector<SimpleParamTable::Row>& rows2)
    {
...
TEST_F(ExplicitSimpleParamTest, readWithoutCondition)
{
    ...
    std::vector<SimpleParamTable::Row> writtenRows;
    ...
    std::vector<SimpleParamTable::Row> readRows;
