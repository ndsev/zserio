...
class NumBitsOperatorTest(Expressions.TestCase):
    def testGetNumBits8(self):
        ...
        for value8 in range(1, 256):
            ...
            self.assertEqual(NumBitsOperatorTest._calcNumBits(value8), numBitsFunctions.get_num_bits8())

    def testGetNumBits16(self):
        numBitsFunctions = self.api.NumBitsFunctions()
        for value16 in range(1, 65536):
            numBitsFunctions.value16 = value16
            self.assertEqual(NumBitsOperatorTest._calcNumBits(value16), numBitsFunctions.get_num_bits16())
