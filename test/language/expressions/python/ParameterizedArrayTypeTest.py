...
class ParameterizedArrayTypeTest(Expressions.TestCase):
    def testBitSizeOfWithOptional(self):
        array = [
            self.api.ParameterizedArrayElement(needs_extra_=False, value1_=0),
        ...
        parameterizedArrayTypeExpression = self.api.ParameterizedArrayTypeExpression(
            parameterizedArrayHolder, True
        )

        self.assertEqual(
            self.PARAMETERIZED_ARRAY_TYPE_EXPRESSION_BIT_SIZE_WITH_OPTIONAL,
    ...
    def testBitSizeOfWithoutOptional(self):
        ...
        parameterizedArrayHolder = self.api.ParameterizedArrayHolder(False, array)
        parameterizedArrayTypeExpression = self.api.ParameterizedArrayTypeExpression(
            parameterizedArrayHolder, False
        )

        self.assertEqual(
            self.PARAMETERIZED_ARRAY_TYPE_EXPRESSION_BIT_SIZE_WITHOUT_OPTIONAL,
            parameterizedArrayTypeExpression.bitsizeof(),
        )
