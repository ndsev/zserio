...
TEST(TemplatedStructRecursionTest, writeRead)
{
    ...
    templatedStructRecursion.getRecursiveTemplate().setData(
            vector_type<uint32_t>{{1, 2, 3}});
    templatedStructRecursion.getRecursiveTemplate().setRecursiveTemplate(
            RecursiveTemplate_uint32{
                    vector_type<uint32_t>{2, 3, 4},
                    RecursiveTemplate_uint32{
                            vector_type<uint32_t>{}, // lengthof(data) == 0 -> end of recursion
                            zserio::NullOpt
                    }
            }
    );
